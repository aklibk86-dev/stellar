...
支持为整个站点配置自定义背景媒体，包括静态图片和视频两种类型。背景媒体会响应式地覆盖整个视口，并可在媒体之上叠加半透明遮罩以提升前景内容的可读性。视频背景支持自动播放、循环与静音配置。相关配置位于 `public/env.js` 的 `background` 字段，修改后无需重新构建。

### 2. 全局毛玻璃卡片特效

启用后，所有使用 `.stellar-glass` 或 `.stellar-card` 类的卡片组件将应用 `backdrop-filter` 模糊与半透明背景的毛玻璃效果。模糊强度、背景透明度、边框样式与圆角等参数均通过 CSS 变量驱动，可在运行时动态调整。相关配置位于 `public/env.js` 的 `glassmorphism` 字段。

### 3. 套餐页面详情渲染修复

修复了套餐购买页面中富文本详情无法正确渲染的问题。现在同时支持 HTML 和 Markdown 格式，混合内容也能正确解析，并经过 DOMPurify 净化以降低跨站脚本风险。标题、列表、代码块、表格、链接等语法均可正常显示。

### 4. 知识库文档渲染

知识库页面统一使用 `marked` 解析 Markdown，并叠加 DOMPurify 净化。纯 HTML、纯 Markdown 以及 HTML 与 Markdown 混排内容均可正确渲染，折叠块、行内标签、代码块等场景不再被错误识别为纯 HTML 而跳过 Markdown 解析。
